...
function Manager() {
    ...
    const addChat = (modecontext: string, chatList: ChatItem[]) => {
        ...
        // 暂停对话的信息
        console.log("新增会话", stopSession)
        localStorage.setItem(newItem.id+"stop", JSON.stringify(stopSession));
        // 暂停情况
        const new_models = models?.slice()
        for (let i = 0; i < numOfModel!; i++) {
            if (models){
