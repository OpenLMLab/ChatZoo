...
import { Form, Input, message, Modal, Radio, RadioChangeEvent, Select, Tooltip } from 'antd';
...
const Banner: React.FC<BannerProps> = ({ model, index, models, handleSwitchLayout }) => {
    ...
    const stopModelSse = (index: number, models: ModelConfig[]) => {
        ...
        newModels[index].start = false;
        console.log(newModels);
        setModels?.setModels(newModels);
    };
    ...
    // evaluation 模式下，需要隐藏一些内容
    let sysMode = false
    let initDsName = ''
    if (localStorage.getItem("sys_mode") === 'evaluation') {
        sysMode = true
        initDsName = localStorage.getItem('selectDs')!
    }
    const [dsName, setDsName] = useState<string>(initDsName)

    const [modeSwitch, setModeSwitch] = useState(sysMode)
    const [filterSwitch, setFilterSwitch] = useState(false)
    const [defaultValue, setValue] = useState(['default', 'default', 'default', 'default']);
    const [filterContent, setFilterContent] = useState<any>({})
    const [messageApi, contextHolder] = message.useMessage();
    const filterFn = (nickname: string, idx: number) => (e: RadioChangeEvent) => {
        // 数据筛选下，选中某个 radio 框
        let newValue = defaultValue.slice()
        newValue[idx] = e.target.value
        setValue(newValue)
        filterContent[nickname] = e.target.value
    ...
    const onFilterOKFn = () => {
        // 数据筛选点击确认
        // 首先确认数据是否完整
        if (Object.keys(filterContent).length != Object.keys(models).length) {
            messageApi.open({
                type: 'error',
                content: '提交前请先选中选项',
            });
        ...
        eventBus.emit("getFilterPageNum", filterContent)
        eventBus.emit("eval_page_change", 1, 0)
    ...
    useEffect(() => {
        // 用于更改 dsName 变量
        const onDsNameChange = (newDsName: string) => {
            setDsName(newDsName)
        }
        eventBus.on('bannerDsNameChange', onDsNameChange)
        return () => {
            eventBus.off('bannerDsNameChange', onDsNameChange)
        }
    })
    ...
    return (
        <>
            <Modal
                ...
                onOk={() => { setModalConfig(newConfig); setOpenModelConfig(false); eventBus.emit("modifyModels", newConfig, index); console.log(modalConfig) }}
            ...
            >
                <div className={styles.modelConfigItem}>
                    ...
                    <Select
                        ...
                        style={{ width: 120 }}
                        onChange={(event) => { newConfig["stream"] = event; console.log(newConfig) }}
                        options={[
                ...
                <div className={styles.modelConfigItem}>
                    ...
                    <TextArea rows={4} placeholder={model.prompts.meta_prompt || "meta_prompt is"} maxLength={6} onChange={(event) => {
                        newConfig.prompts["meta_prompt"] = event.target.value
                    }} />
                </div>
                ...
                <div className={styles.modelConfigItem}>
                    ...
                    <TextArea rows={4} placeholder={model.prompts.user_prompt || "user prompt is"} maxLength={6} onChange={(event) => {
                        newConfig.prompts["user_prompt"] = event.target.value
                    }} />
                </div>
                ...
                <div className={styles.modelConfigItem}>
                    ...
                    <TextArea rows={4} placeholder={model.prompts.bot_prompt || "bot prompt is"} maxLength={6} onChange={(event) => {
                        newConfig.prompts["bot_prompt"] = event.target.value
                    }} />
                </div>
                        ...
                        <div key={key}>
                            ...
                            <Input
                                ...
                                onChange={(event) => {
                                    ...
                                    // @ts-ignore
                                    newConfig.generate_kwargs[key] = updatedValue
                                    console.log('修改后的配置', newConfig)
                                }}
                            />
                        </div>
                    ))}
            ...
            <Modal width={520} title="数据筛选" open={filterSwitch} onOk={onFilterOKFn}
                ...
                <div className={styles.filterWrap}>
                    <Form
                        ...
                        style={{ minWidth: 500 }}
                    >
                        {models.map((model: ModelConfig, idx: number) => {
                            return <Form.Item label={model.nickname}>
                                <Radio.Group onChange={filterFn(model.nickname, idx)} value={defaultValue[idx]}>
                                    <Radio value="correct"> 答案正确 </Radio>
                                    <Radio value="error"> 答案错误 </Radio>
                                    <Radio value="pass"> 默认 </Radio>
            ...
            {
                modeSwitch ?
                    <Tooltip title={modelName + "/" + dsName}>
                        <div className={styles.typo}>{modelName + "/" + dsName}</div>
                    </Tooltip>
            ...
            <div className={styles.func}>
                ...
                {
                    modeSwitch ?
                        <Tooltip title={<span className={styles.tooltipTitle} >筛选数据</span>}>
                            <div onClick={() => { setFilterSwitch(true) }}
                            >
                                <FilterOutlined />
                            </div>
                        </Tooltip>
